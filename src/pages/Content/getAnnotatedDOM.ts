import { MICROTOY_ELEMENT_SELECTOR } from '../../constants';
...
export function getUniqueElementSelectorId(id: number): string {
  ...
  let uniqueId = element.getAttribute(MICROTOY_ELEMENT_SELECTOR);
  if (uniqueId) return uniqueId;
  uniqueId = Math.random().toString(36).substring(2, 10);
  element.setAttribute(MICROTOY_ELEMENT_SELECTOR, uniqueId);
  return uniqueId;
}
